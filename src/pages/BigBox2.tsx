...
const PROMISES = [
  {
    ...
    proof: "Подтверждено на 12 объектах за 2022–2024 гг. Среднее значение — 118 дней.",
    icon: "CalendarCheck",
  },
  {
    claim: "5 000 м²",
    sub: "монтаж в месяц при стандартном проекте",
    proof: "Достигнуто на объекте в Воронежской области (45 000 м², 2023). Бригада 24 чел.",
    icon: "TrendingUp",
  },
  {
    claim: "0 срывов",
    sub: "сроков по договорным объектам за 3 года",
    proof: "Выборка: 47 объектов с фиксированным сроком сдачи в договоре. Период: 2021–2024.",
    icon: "ShieldCheck",
  },
];
...
const CASES = [
  ...
  },
  {
    ...
    metrics: ["112 дней до РНС", "Монтаж 4 800 м²/мес.", "−8% к плановому бюджету"],
...
function LeadForm({ type, onClose, onSuccess }: { type: FormType; onClose: () => void; onSuccess: () => void; }) {
      ...
      <div className="bg-white w-full max-w-lg relative shadow-2xl">
        ...
        <form onSubmit={(e) => { e.preventDefault(); onSuccess(); }} className="p-6 space-y-4">
          {[
            ...
            { label: "E-mail*", key: "email", type: "email", placeholder: "ivan@company.ru" },
          ].map((f) => (
            <div key={f.key}>
              <label className="font-ibm text-xs text-evraz-gray uppercase tracking-wider block mb-1.5">{f.label}</label>
              <input
          ...
          <button type="submit" className="w-full bg-evraz-red text-white font-oswald text-sm tracking-widest uppercase py-4 hover:bg-red-700 transition-colors">
            {isCatalog ? "ПОЛУЧИТЬ КАТАЛОГ" : "ЗАПРОСИТЬ РАСЧЁТ"}
          </button>
          <p className="font-ibm text-xs text-evraz-gray text-center">
            Нажимая кнопку, вы соглашаетесь с{" "}
            <a href="#" className="underline hover:text-evraz-dark">политикой конфиденциальности</a>
          </p>
        </form>
      </div>
    </div>
...
export default function BigBox2() {
  ...
  return (
    <div className="min-h-screen bg-white font-ibm">
      {activeForm && (
        <LeadForm type={activeForm} onClose={() => setActiveForm(null)} onSuccess={handleSuccess} />
      )}
        ...
        <div className="container mx-auto py-4 flex items-center justify-between gap-4">
          <button onClick={() => navigate("/")} className="flex items-center gap-3 shrink-0">
            ...
            <div>
              ...
              <div className="font-ibm text-xs text-gray-400 tracking-wider uppercase">BigBox</div>
            </div>
          </button>
      ...
      <section id="hero" className="bg-evraz-dark min-h-screen flex flex-col justify-end pb-0 relative overflow-hidden">
        ...
        <div className="container mx-auto pt-36 pb-0 relative z-10">
          ...
          <h1 className="font-oswald text-4xl md:text-6xl lg:text-7xl text-white font-bold leading-[1.05] mb-8 max-w-4xl">
            ПРОМЫШЛЕННОЕ ЗДАНИЕ
            <br />
            <span className="text-evraz-red">ОТ КОНЦЕПЦИИ</span>
            <br />
            ДО РАЗРЕШЕНИЯ — 4 МЕСЯЦА
          </h1>
            ...
            {PROMISES.map((p) => (
              <div key={p.claim} className="bg-evraz-dark p-6 relative group">
                <div className="flex items-start gap-4 mb-4">
                  ...
                  <div>
                    <div className="font-oswald text-3xl text-white font-bold leading-none">{p.claim}</div>
                    <div className="font-ibm text-xs text-gray-400 mt-1 leading-relaxed">{p.sub}</div>
                  </div>
                </div>
                {/* Доказательство */}
                <div className="border-t border-white/10 pt-4">
                  <div className="flex items-start gap-2">
                    <Icon name="FileCheck" size={12} className="text-evraz-red mt-0.5 shrink-0" />
                    <p className="font-ibm text-xs text-gray-500 leading-relaxed italic">{p.proof}</p>
                  </div>
                </div>
              </div>
            ))}
      ...
      <section id="cases" className="py-20 bg-evraz-light">
        <div className="container mx-auto">
          ...
          <p className="font-ibm text-evraz-gray text-sm mb-10 max-w-2xl leading-relaxed">
            Каждый кейс — с адресом объекта, датами и фактическими метриками. B2B-клиент оценивает доказательства, а не слова.
          </p>
          ...
          <div className="mt-8 bg-white border border-evraz-border p-5 flex items-start gap-3">
            <Icon name="Info" size={16} className="text-evraz-gray shrink-0 mt-0.5" />
            <p className="font-ibm text-xs text-evraz-gray leading-relaxed">
              Полные данные по объектам (акты КС-2, исполнительная документация, фото этапов) предоставляем по запросу на встрече.
              Клиент вправе требовать верификации любой заявленной метрики.
            </p>
          </div>
        </div>
      </section>
      ...
      <section id="challenge" className="py-20 bg-evraz-dark relative overflow-hidden">
        ...
        <div className="container mx-auto relative z-10">
          <div className="flex items-center gap-3 mb-3">
            <div className="w-10 h-0.5 bg-evraz-red" />
            <span className="font-oswald text-evraz-red text-xs tracking-[0.25em] uppercase">Цена ошибки</span>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div>
              ...
              <div className="space-y-4">
                {[
                  { risk: "Срыв сроков сдачи", stat: "+3–6 мес.", detail: "Среднее отклонение по рынку промстроительства (данные ДОМ.РФ, 2023)" },
                  { risk: "Перерасход бюджета", stat: "+18–25%", detail: "Переделки из-за несогласованности проекта и поставки" },
                  { risk: "Конфликт зон ответственности", stat: "47% споров", detail: "Доля судебных дел по промстройке, где причина — размытая ответственность" },
                  { risk: "Вынужденный простой производства", stat: "от 2 мес.", detail: "Пока заказчик разбирается с подрядчиками, оборудование стоит" },
                ].map((r) => (
                  <div key={r.risk} className="flex gap-4 bg-white/5 border border-white/10 p-4">
                    <div className="shrink-0">
                      <div className="font-oswald text-2xl text-evraz-red font-bold leading-none">{r.stat}</div>
                    </div>
                    <div>
                      <div className="font-oswald text-sm text-white uppercase tracking-wider mb-1">{r.risk}</div>
                      <div className="font-ibm text-xs text-gray-500 leading-relaxed italic">{r.detail}</div>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="bg-evraz-red/10 border border-evraz-red/30 p-6 flex flex-col justify-between">
              <div>
                ...
                <div className="space-y-4">
                  {[
                    { fix: "Единый договор", desc: "Проектирование, поставка и монтаж — один контрагент, одна точка ответственности" },
                    { fix: "Фиксированная цена", desc: "Смета закрыта до начала работ. Мы несём риск стоимости материалов, а не вы" },
                    { fix: "График в договоре", desc: "Штрафные санкции за каждый день просрочки — мы заинтересованы в сроке так же, как вы" },
                    { fix: "Параллельные процессы", desc: "Проектирование и производство конструкций идут одновременно — это даёт +4–6 недель к скорости" },
                  ].map((f) => (
                    <div key={f.fix} className="flex items-start gap-3">
                      ...
                      <div>
                        <div className="font-oswald text-sm text-white uppercase tracking-wide">{f.fix}</div>
                        <div className="font-ibm text-xs text-gray-400 mt-0.5 leading-relaxed">{f.desc}</div>
                      </div>
                    </div>
                  ))}
                </div>
      ...
      <section id="how" className="py-20 bg-white">
        <div className="container mx-auto">
          <div className="flex items-center gap-3 mb-3">
            <div className="w-10 h-0.5 bg-evraz-red" />
            <span className="font-oswald text-evraz-red text-xs tracking-[0.25em] uppercase">Процесс</span>
          </div>
          ...
          <p className="font-ibm text-evraz-gray text-sm mb-10 max-w-xl leading-relaxed">
            Не за счёт аврала — за счёт параллельных процессов и единого управления.
          </p>

          <div className="max-w-4xl space-y-0 border-l-2 border-evraz-border ml-7">
            {[
              {
                num: "01", icon: "Lightbulb", title: "Техническое задание", duration: "1–3 дня",
              ...
              {
                num: "02", icon: "FileText", title: "Проектирование", duration: "2–4 недели",
                desc: "Готовим рабочую документацию по ГОСТ. Вы согласовываете решения до производства — никаких переделок в процессе.",
                parallel: "Параллельно: запускаем заготовительное производство на собственном заводе",
              },
              {
                num: "03", icon: "Factory", title: "Производство конструкций", duration: "4–8 недель",
                desc: "Металлоконструкции изготавливаем на заводах EVRAZ. Пооперационный контроль, маркировка каждого элемента.",
                parallel: "Параллельно: подготовка площадки и фундаментные работы на объекте",
              },
              {
                num: "04", icon: "Truck", title: "Доставка на площадку", duration: "По графику",
              ...
              {
                num: "05", icon: "HardHat", title: "Монтаж и сдача", duration: "6–16 недель",
                desc: "Аккредитованная бригада, поэтапная приёмка. Сдаём с исполнительной документацией и гарантийным письмом.",
                parallel: null,
              },
            ].map((s, i, arr) => (
              <div key={s.num} className="flex gap-6 relative pl-8 pb-10 last:pb-0">
                {/* Точка на линии */}
                ...
                <div className="flex-1">
                  <div className="flex flex-wrap items-center gap-3 mb-2">
                    ...
                    <span className="font-oswald text-xs text-evraz-gray tracking-widest">ЭТАП {s.num}</span>
                    <span className="font-ibm text-xs text-evraz-red font-medium">{s.duration}</span>
                  </div>
                  <h4 className="font-oswald text-lg text-evraz-dark font-semibold mb-1">{s.title}</h4>
                  <p className="font-ibm text-sm text-evraz-gray leading-relaxed mb-2">{s.desc}</p>
                  {s.parallel && (
                    <div className="flex items-center gap-2 bg-evraz-red/5 border border-evraz-red/20 px-3 py-2 w-fit">
                      <Icon name="Zap" size={11} className="text-evraz-red shrink-0" />
                      <span className="font-ibm text-xs text-evraz-dark leading-relaxed">{s.parallel}</span>
                    </div>
                  )}
                </div>
              </div>
      ...
      <section id="trust" className="py-20 bg-evraz-light">
        <div className="container mx-auto">
          <div className="flex items-center gap-3 mb-3">
            <div className="w-10 h-0.5 bg-evraz-red" />
            <span className="font-oswald text-evraz-red text-xs tracking-[0.25em] uppercase">Технические решения</span>
          </div>
          <h2 className="font-oswald text-3xl md:text-4xl text-evraz-dark font-semibold mb-3">
          ...
          <p className="font-ibm text-evraz-gray text-sm mb-8 max-w-2xl leading-relaxed">
            Пролёты до 100 м без промежуточных опор, кровля и ограждающие конструкции под любой климат,
            специальные решения для опасных производств. Вся сталь — собственного производства EVRAZ, качество подтверждено сертификатами.
          </p>
          ...
          <div className="bg-evraz-dark p-8 md:p-10 flex flex-col md:flex-row items-center justify-between gap-6">
            <div>
              ...
              <p className="font-ibm text-sm text-gray-400">
                Конструкционные решения EVRAZ STEEL BOX — выберите подходящий вариант и оцените экономию ещё до начала проектирования
              </p>
            </div>
            <button onClick={() => setActiveForm("catalog")}
      ...
      <section className="py-20 bg-white">
        <div className="container mx-auto">
          <div className="flex items-center gap-3 mb-10">
            <div className="w-10 h-0.5 bg-evraz-red" />
            <span className="font-oswald text-evraz-red text-xs tracking-[0.25em] uppercase">Факты о компании</span>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-0 border border-evraz-border">
            {[
              ...
              { value: "✓", label: "Собственное производство металлоконструкций" },
              { value: "EVRAZ", label: "Часть группы EVRAZ" },
            ].map((s, i) => (
              <div key={i} className="p-8 text-center border-r border-evraz-border last:border-r-0">
      ...
      {/* ── CTA ФИНАЛЬНЫЙ ── */}
      <section id="cta" className="py-20 bg-evraz-dark relative overflow-hidden">
        <div className="absolute left-0 top-0 bottom-0 w-1 bg-evraz-red" />
        <div className="absolute inset-0 opacity-[0.04]"
          style={{ backgroundImage: "repeating-linear-gradient(0deg,transparent,transparent 60px,#fff 60px,#fff 61px),repeating-linear-gradient(90deg,transparent,transparent 60px,#fff 60px,#fff 61px)" }} />
        <div className="container mx-auto relative z-10">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
            <div>
              <div className="flex items-center gap-3 mb-4">
                <div className="w-10 h-0.5 bg-evraz-red" />
                <span className="font-oswald text-evraz-red text-xs tracking-[0.25em] uppercase">Зафиксировать условия</span>
              </div>
              ...
              <p className="font-ibm text-gray-400 text-sm leading-relaxed mb-6">
                Оставьте заявку — мы перезвоним и разберём вашу задачу: тип объекта, площадь, регион и бюджет. Расчёт получите в течение одного рабочего дня.
              </p>
              ...
              <div className="flex flex-wrap gap-4">
                <a href="https://t.me/EvrazBigBox_bot" target="_blank" rel="noreferrer"
                  className="flex items-center gap-2 font-ibm text-xs text-gray-400 hover:text-white transition-colors">
                  <Icon name="Send" size={13} className="text-evraz-red" />@EvrazBigBox_bot
                </a>
                <a href="https://wa.me/79692688420" target="_blank" rel="noreferrer"
                  className="flex items-center gap-2 font-ibm text-xs text-gray-400 hover:text-white transition-colors">
                  <Icon name="MessageCircle" size={13} className="text-evraz-red" />+7 969 268-84-20
                </a>
              </div>
            </div>
            ...
            <div className="bg-evraz-light p-8">
              <h3 className="font-oswald text-lg text-evraz-dark font-semibold mb-6">ОСТАВИТЬ ЗАЯВКУ</h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-8">
            <div>
              ...
              <p className="font-ibm text-xs text-gray-400 leading-relaxed mb-3">
                Полнокомплектные здания площадью от 3 000 м²<br />Работаем во всех регионах России
              </p>
              <a href="tel:+78003006559" className="font-oswald text-xl text-white hover:text-evraz-red transition-colors block mb-1">
                8 800 300 65 59
              </a>
              ...
              <div className="mt-4 space-y-2">
                {[
                  { href: "https://t.me/EvrazBigBox_bot", icon: "Send", label: "@EvrazBigBox_bot" },
                  { href: "https://wa.me/79692688420", icon: "MessageCircle", label: "+7 969 268-84-20" },
                  { href: "mailto:sales.bigbox@evrazsteel.ru", icon: "Mail", label: "sales.bigbox@evrazsteel.ru" },
                ].map((c) => (
                  <a key={c.label} href={c.href} target="_blank" rel="noreferrer"
                    className="flex items-center gap-2 font-ibm text-xs text-gray-400 hover:text-white transition-colors">
                    <Icon name={c.icon} size={13} className="text-evraz-red" />{c.label}
                  </a>
                ))}
              </div>
            </div>
            <div>
              <h5 className="font-oswald text-xs text-gray-400 uppercase tracking-widest mb-4">Навигация</h5>
              <ul className="space-y-2">
                {NAV_LINKS.map((l) => (
                  <li key={l.label}>
                    <button onClick={() => scrollTo(l.href)}
                      className="font-ibm text-sm text-gray-400 hover:text-white transition-colors">{l.label}</button>
                  </li>
                ))}
              </ul>
            </div>
            <div>
              <h5 className="font-oswald text-xs text-gray-400 uppercase tracking-widest mb-4">Документы</h5>
              <ul className="space-y-2">
                <li><a href="#" className="font-ibm text-sm text-gray-400 hover:text-white transition-colors">Политика конфиденциальности</a></li>
                <li><a href="#" className="font-ibm text-sm text-gray-400 hover:text-white transition-colors">Согласие на обработку персональных данных</a></li>
              </ul>
            </div>
          </div>
          <div className="border-t border-evraz-charcoal pt-6">
            <p className="font-ibm text-xs text-gray-500">© {new Date().getFullYear()} EVRAZ Steel Box. Все права защищены.</p>
          </div>
        </div>
      </footer>
    </div>
...
function InlineForm({ onSuccess }: { onSuccess: () => void }) {
  ...
  return (
    <form onSubmit={(e) => { e.preventDefault(); onSuccess(); }} className="space-y-4">
      ...
      <div>
        <label className="font-ibm text-xs text-evraz-gray uppercase tracking-wider block mb-1.5">Задайте вопрос</label>
